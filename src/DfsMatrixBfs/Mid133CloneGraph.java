...
public class Mid133CloneGraph {
	...
	public Node cloneGraph(Node node) {
        ...
        //using bfs to get all the neighbors & all it's neighbors
...
class Solution {
    public Node cloneGraph(Node node) {
        ...
        Map<Node, Node> map = new HashMap<>();
        map.put(node, new Node(node.val, new ArrayList<>()));
        //using dfs to get all the neighbors & all it's neighbors
        dfs(map, node);
        return map.get(node);
    }
    
    private void dfs(Map<Node, Node> map, Node cur) {
        List<Node> nei = cur.neighbors;
        for(Node n : nei) {
            if(!map.containsKey(n)) {
                map.put(n, new Node(n.val, new ArrayList<>()));
